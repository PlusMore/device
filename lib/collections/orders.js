...
Schema.Order = new SimpleSchema({
  ...
  // reservations are for external restaurants/clubs
  reservation: {
    type: Object,
    optional: true,
    blackbox: true
  },
  // purchases are for tangible items (room service food, etc)
  purchase: {
    type: Object,
    optional: true,
    blackbox: true
  },
  // services are currently open to interpretation. Internal hotel services.
  // Possible uses are for hotel services, such as valet, spa, laundry
  service: {
    type: Object,
    optional: true,
    blackbox: true
...
Meteor.methods({
  ...
  requestService: function(service) {
    ...
    if (Meteor.isServer) {
      ...
      var friendlyRequestType = HotelServices.friendlyRequestType(service.type);
